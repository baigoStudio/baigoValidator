        ...
        */
        var verifyReg = function(_str, _format) {
            switch(_format) {
                ...
                break;
                case "strDigit":
                    var _reg = /^[\u4e00-\u9fa50-9A-Za-z_]+$/; // "/^[\u4e00-\u9fa5|\uf900-\ufa2d|\w]*$/" 字母中文数字下划线减号
                break;
                case "ip":
        ...
        */
        ...
        var verifyStr = function(_validate_obj) {
            ...
            if (typeof _validate.selector == "undefined") { //如果 validate 对象没有定义选择器，则默认将一级对象名作为 id 选择器
                var _validate_selector = "#" + _validate_obj;
            } else {
                var _validate_selector = _validate.selector; //待验证选择器
            }
            var _result;
            //alert(_validate.type);
            switch (_validate.type) {
                case "digit":
                    var _str   = $(_validate_selector).val(); //获取表单值
                    _status    = isDigit(_str, _len, _validate.format); //验证字符串
                break;
                case "radio":
                case "checkbox":
                    var _num   = $(_validate_selector + ":checked").size(); //获取表单选中数
                    _status    = isNum(_num, _len); //验证个数
                break;
                case "select":
                    if (_len.min > 1) { //如最少大于 1 为多选，则验证个数
                        var _num  = $(_validate_selector + " :selected").size(); //获取表单选中数
                        _status   = isNum(_num, _len); //验证个数
                    } else { //单选则验证值
                        var _str  = $(_validate_selector).val(); //获取表单值
